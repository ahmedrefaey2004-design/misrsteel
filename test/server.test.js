...
test('admin portfolio CRUD and generic site-config collection endpoints work', async () => {
  await withServer({ adminToken: 'admin_portfolio_crud' }, async (baseUrl) => {
    ...
    const autoIdCreateRes = await fetch(`${baseUrl}/api/admin/portfolio`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ name: 'Project Auto Id', category: 'hall' })
    });
    assert.equal(autoIdCreateRes.status, 201);
    const autoIdCreateJson = await autoIdCreateRes.json();
    const generated = autoIdCreateJson.projects.find((p) => p.name === 'Project Auto Id');
    assert.equal(Number.isFinite(Number(generated.id)), true);
    ...
    assert.equal(deleteJson.projects.length, 1);
  });
});
...
test('admin stats endpoint returns dashboard KPIs', async () => {
  await withServer({ adminToken: 'admin_stats_token' }, async (baseUrl) => {
    ...
    const affiliateRegisterRes = await fetch(`${baseUrl}/api/affiliates/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: 'Stats Affiliate',
        phone: '+201555555555'
      })
    });
    assert.equal(affiliateRegisterRes.status, 201);
    const affiliateRegisterJson = await affiliateRegisterRes.json();
    const affiliateCode = affiliateRegisterJson.affiliate.code;

    const orderRes = await fetch(`${baseUrl}/api/orders`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        customerName: 'Stats Order',
        customerPhone: '+201666666666',
        items: [{ id: 'p-stats', nameAr: 'منتج', qty: 1, priceUSD: 120 }],
        totalUSD: 120,
        affiliateCode
      })
    });
    ...
    const statsRes = await fetch(`${baseUrl}/api/admin/stats`, {
      headers: { 'x-admin-token': 'admin_stats_token' }
    });
    assert.equal(statsRes.status, 200);
    const statsJson = await statsRes.json();
    assert.equal(statsJson.success, true);
    assert.equal(statsJson.stats.totalRevenue, 120);
    assert.equal(statsJson.stats.newOrders, 1);
    assert.equal(statsJson.stats.totalCustomers, 1);
    assert.equal(statsJson.stats.pendingCommissions, 12);
  });
});

test('admin customers endpoint lists and filters customers', async () => {
  await withServer({ adminToken: 'admin_customers_token' }, async (baseUrl) => {
    const registerRes = await fetch(`${baseUrl}/api/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    const listRes = await fetch(`${baseUrl}/api/admin/customers`, {
      headers: { 'x-admin-token': 'admin_customers_token' }
    });
    assert.equal(listRes.status, 200);
    const listJson = await listRes.json();
    assert.equal(listJson.success, true);
    assert.equal(listJson.total, 1);
    assert.equal(listJson.customers[0].email, 'customer.one@example.com');

    const filterRes = await fetch(`${baseUrl}/api/admin/customers?q=customer.one`, {
      headers: { 'x-admin-token': 'admin_customers_token' }
    });
...
test('admin settings endpoint reads and updates dashboard settings', async () => {
  await withServer({ adminToken: 'admin_settings_token' }, async (baseUrl) => {
    const headers = {
      'Content-Type': 'application/json',
      'x-admin-token': 'admin_settings_token'
    };

    const getRes = await fetch(`${baseUrl}/api/admin/settings`, { headers });
    assert.equal(getRes.status, 200);
    const getJson = await getRes.json();
    assert.equal(getJson.success, true);
    assert.equal(getJson.settings.profitMargin, 25);
    ...
    assert.equal(putJson.settings.usdRate, 52.5);
    assert.equal(putJson.settings.profitMargin, 30);

    const publicRes = await fetch(`${baseUrl}/api/settings/public`);
    assert.equal(publicRes.status, 200);
    const publicJson = await publicRes.json();
    assert.equal(publicJson.usdRate, 52.5);
  });
});

test('admin affiliates endpoints list/create/update/delete affiliates', async () => {
  await withServer({ adminToken: 'admin_affiliates_token' }, async (baseUrl) => {
    const headers = {
      'Content-Type': 'application/json',
      'x-admin-token': 'admin_affiliates_token'
    };

    const createRes = await fetch(`${baseUrl}/api/admin/affiliates`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
    ...
    assert.equal(createRes.status, 201);
    const createJson = await createRes.json();
    const affiliateId = createJson.affiliate.id;
    const affiliateCode = createJson.affiliate.code;
    assert.ok(affiliateId);
    assert.ok(affiliateCode);

    const orderRes = await fetch(`${baseUrl}/api/orders`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        customerName: 'Affiliate Order',
        items: [{ id: 'p-aff-admin', nameAr: 'منتج', qty: 1, priceUSD: 100 }],
        totalUSD: 100,
        affiliateCode
      })
    });
    ...
    const listRes = await fetch(`${baseUrl}/api/admin/affiliates`, { headers });
    assert.equal(listRes.status, 200);
    const listJson = await listRes.json();
    assert.equal(listJson.total, 1);
    assert.equal(listJson.affiliates[0].totalSales, 100);

    const updateRes = await fetch(`${baseUrl}/api/admin/affiliates/${affiliateId}`, {
      method: 'PUT',
      headers,
    ...
    assert.equal(updateRes.status, 200);
    const updateJson = await updateRes.json();
    assert.equal(updateJson.affiliate.status, 'inactive');

    const payoutRes = await fetch(`${baseUrl}/api/admin/affiliates/${affiliateId}/commission`, {
      method: 'PUT',
      headers,
    ...
    const deleteRes = await fetch(`${baseUrl}/api/admin/affiliates/${affiliateId}`, {
      method: 'DELETE',
      headers
    });
    assert.equal(deleteRes.status, 200);
