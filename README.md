...
- لو قابلتك مشكلة push بصلاحيات 403 راجع: `PUSH_TROUBLESHOOTING_AR.md`.
...
### 4) لو عندك ملف صور مضغوط (ZIP) لقسم جديد
تقدر تضيف القسم + المنتجات تلقائيًا بالأمر ده:

```bash
npm run import:section:zip -- \
  --zip /path/to/section-images.zip \
  --section-id wedding-hall \
  --section-ar "قاعات أفراح" \
  --section-en "Wedding Halls" \
  --category chairs \
  --price 52
```

- السكريبت هيعمل:
  1. فك ضغط الصور داخل `images/<section-id>/`
  2. إضافة القسم في `sections` لو مش موجود
  3. إنشاء منتجات تلقائيًا داخل `products`
  4. تحديث `data/site-config.json`

وبعدها شغّل الرفع للـ API:
```bash
ADMIN_TOKEN=your_strong_token npm run upload:products -- --replace
```
