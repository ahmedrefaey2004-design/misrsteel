...
const config = {
  ...
  allowedOrigins: process.env.ALLOWED_ORIGINS
    ...
    : ['*'],
  maxPromptLength: Number.parseInt(process.env.MAX_PROMPT_LENGTH || '1200', 10),
  generateRateLimit: Number.parseInt(process.env.GENERATE_RATE_LIMIT || '30', 10),
  generateRateWindowMs: Number.parseInt(process.env.GENERATE_RATE_WINDOW_MS || String(60 * 1000), 10)
};
...
function hitRateLimit(key, maxRequests, windowMs) {
  const now = Date.now();
...
app.post('/api/generate', requireJsonBody, async (req, res) => {
  ...
  if (hitRateLimit(rateKey, config.generateRateLimit, config.generateRateWindowMs)) {
...
const server = app.listen(config.port, () => {
  console.log(`MISR STEEL v2.2 on port ${config.port}`);
