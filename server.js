...
function createApp(config = createConfig()) {
  ...
  app.get('/api/admin/settings', (req, res) => {
    ...
    const settings = {
      ...defaults,
      ...(configValue.settings && typeof configValue.settings === 'object' && !Array.isArray(configValue.settings)
  ...
  app.put('/api/admin/settings', (req, res) => {
    ...
    const current = siteConfigStore.get();
    const currentSettings = current.settings && typeof current.settings === 'object' && !Array.isArray(current.settings)
      ? current.settings
      : {};
    const nextSettings = { ...currentSettings };
    const allowed = ['usdRate', 'phone', 'email', 'profitMargin', 'discount100', 'discount500'];
    for (const key of allowed) {
      if (Object.prototype.hasOwnProperty.call(req.body, key)) {
        nextSettings[key] = req.body[key];
      }
    }
    if (Number.isFinite(Number(nextSettings.usdRate)) && Number(nextSettings.usdRate) > 0) {
      current.usdRate = Number(nextSettings.usdRate);
      nextSettings.usdRate = Number(nextSettings.usdRate);
    }
    current.settings = nextSettings;
    const updated = siteConfigStore.replace(current);

    return res.json({ success: true, settings: updated.settings || {} });
  });
